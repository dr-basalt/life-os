<script lang="ts">
  ...
  onMount(async () => {
    try {
      const records = await pb.collection('onboarding_profile').getList(1, 1)
      if (records.totalItems > 0) {
        const r = records.items[0]
        existingProfileId = r.id
        prenom = r.prenom || ''
        birthDate = r.birth_date || ''
        birthTime = r.birth_time || ''
        situation = r.situation || 'salarié'
        revenus = r.revenus_mensuels || 0
        heures = r.heures_dispo || 10
        competences = JSON.parse(r.competences || '[]')
        revePro = r.reve_pro || ''
        mission = r.mission || ''
        selectedDomaines = JSON.parse(r.domaines_prioritaires || '[]')
        p0s = JSON.parse(r.p0_invariants || '[{"label":"","deadline":""}]')
        if (p0s.length === 0) p0s = [{ label: '', deadline: '' }]
        nonVouloir = r.non_vouloir || ''
        // Sauter directement à la vue édition (step 5 = génération)
        step = 5
      }
    } catch {}
    profileLoaded = true
  ...
  async function saveAndGo() {
    ...
    try {
      ...
      // Update si profil existant, create sinon
      if (existingProfileId) {
        await pb.collection('onboarding_profile').update(existingProfileId, profileData).catch(() => {})
      } else {
        const r = await pb.collection('onboarding_profile').create(profileData).catch(() => null)
        if (r) existingProfileId = r.id
      }

      // Supprimer les OKRs existants avant d'en créer de nouveaux
      const existing = await pb.collection('objectives').getFullList()
      for (const obj of existing) {
        const krs = await pb.collection('key_results').getFullList({ filter: `objective = "${obj.id}"` }).catch(() => [])
        for (const kr of krs) await pb.collection('key_results').delete(kr.id).catch(() => {})
        await pb.collection('objectives').delete(obj.id).catch(() => {})
      }

      // Créer les nouveaux OKRs
...
<div class="min-h-screen bg-zinc-950 py-8 px-4">
  <div class="max-w-2xl mx-auto">
    ...
    {:else if existingProfileId && step === 5}
      <!-- ══ MODE PROFIL EXISTANT ══ -->
      <div class="mb-6">
        <div class="text-indigo-400 text-xs font-mono tracking-widest mb-1">STERN OS — COCKPIT</div>
        <h1 class="text-2xl font-bold text-white">Ton profil & roadmap</h1>
        <p class="text-zinc-500 text-sm mt-1">Modifie tes données et régénère ta roadmap à tout moment</p>
      </div>

      <!-- Édition P0 (le plus important) -->
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-5 mb-4">
        <h3 class="text-sm font-semibold text-white mb-1">P0 — Non-négociables</h3>
        <p class="text-zinc-600 text-xs mb-4">Modifie tes invariants fondamentaux</p>
        {#each p0s as p0, i}
          <div class="flex gap-2 mb-2">
            <input bind:value={p0.label} type="text" placeholder={`P0 ${i+1}`}
              class="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white placeholder-zinc-600 focus:outline-none focus:border-violet-500 text-sm" />
      ...
      <!-- Édition ressources & rêve -->
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-5 mb-4">
        <h3 class="text-sm font-semibold text-white mb-4">Contexte & ressources</h3>
        <div class="grid grid-cols-2 gap-4 mb-4">
          <div>
            <label class="text-zinc-500 text-xs uppercase tracking-wider">Situation</label>
          ...
          <div>
            <label class="text-zinc-500 text-xs uppercase tracking-wider">Revenus (€/mois)</label>
            <input bind:value={revenus} type="number" min="0"
              class="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500" />
          </div>
        </div>
        <div class="mb-4">
          <label class="text-zinc-500 text-xs uppercase tracking-wider">Heures dispo/semaine — {heures}h</label>
          <input bind:value={heures} type="range" min="1" max="40" class="w-full mt-2 accent-indigo-500" />
        </div>
        <div class="mb-4">
          <label class="text-zinc-500 text-xs uppercase tracking-wider">Compétences</label>
          <div class="flex gap-2 mt-1">
            <input bind:value={competenceInput} on:keydown={(e) => e.key === 'Enter' && addCompetence()}
              type="text" placeholder="Ajouter..."
              class="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm placeholder-zinc-600 focus:outline-none focus:border-indigo-500" />
            <button on:click={addCompetence} class="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-white text-sm">+</button>
          </div>
          <div class="flex flex-wrap gap-2 mt-2">
            {#each competences as c, i}
              <span class="inline-flex items-center gap-1 bg-indigo-500/15 text-indigo-300 text-xs px-3 py-1 rounded-full">
                {c} <button on:click={() => removeCompetence(i)} class="text-indigo-400 hover:text-red-400 ml-1">×</button>
              </span>
            {/each}
          </div>
        </div>
        <div>
          <label class="text-zinc-500 text-xs uppercase tracking-wider">Rêve pro (5 ans)</label>
          <textarea bind:value={revePro} rows="3"
            class="mt-1 w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-white text-sm resize-none focus:outline-none focus:border-indigo-500"></textarea>
        </div>
      </div>

      <!-- Domaines -->
      <div class="bg-zinc-900 rounded-xl border border-zinc-800 p-5 mb-5">
        <h3 class="text-sm font-semibold text-white mb-3">Domaines prioritaires</h3>
        <div class="grid grid-cols-2 gap-2">
          {#each allDomaines as d}
            <button on:click={() => selectedDomaines.includes(d)
              ...
              class="text-sm px-3 py-2 rounded-lg border text-left transition-colors
      ...
      {#if !loading && !roadmap}
        <button on:click={generateRoadmap}
          class="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-semibold rounded-xl transition-all shadow-lg shadow-indigo-500/20 mb-4">
          🧠 Régénérer ma roadmap (Neo4j + P0)
        </button>
        <button on:click={() => { existingProfileId = null; step = 1 }}
          class="w-full py-2 text-zinc-600 hover:text-zinc-400 text-xs transition-colors">
      ...
      {#if loading}
        <div class="text-center py-10">
          ...
          <p class="text-zinc-400 text-sm">Analyse du persona Neo4j + génération des OKRs...</p>
        </div>
      {/if}
      ...
      {#if roadmap}
        ...
        <div class="flex gap-3">
          <button on:click={() => { roadmap = null }}
            class="flex-1 py-3 border border-zinc-700 hover:border-zinc-500 text-zinc-400 hover:text-white rounded-xl text-sm transition-colors">
            ↺ Régénérer
          </button>
          <button on:click={saveAndGo} disabled={loading}
            class="flex-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold rounded-xl transition-all disabled:opacity-50">
            {loading ? 'Sauvegarde...' : '✅ Valider et accéder au dashboard'}
          </button>
        </div>
      {/if}

    {:else}
      <!-- ══ MODE WIZARD (premier onboarding) ══ -->
        ...
        {:else if step === 2}
          ...
          <div class="mb-5">
            ...
            <div class="flex items-center gap-4 mt-2">
              <input bind:value={heures} type="range" min="1" max="40" class="flex-1 accent-indigo-500" />
